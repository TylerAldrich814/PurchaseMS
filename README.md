...
## Overview
This project is a learning platform to understand and build microservices in Go. It demonstrates how multiple services can work together using modern tools and design patterns to create a scalable and maintainable architecture. The project is designed to showcase key concepts such as service communication, data management, and telemetry in a distributed system.
...
- **Hexagonal Architecture in Payments**: The Payments service follows the hexagonal (port# Go Microservices Project
...
### Planned Workflow (with Stock and Kitchen)
1. When an order is created, the **Orders** service will:
   - Send a gRPC request to the **Stock** service to check item availability.
   - On success, proceed to process payment.
2. After payment, the **Kitchen** service will handle order preparation.
...
4. Use Docker Compose (optional):
   If a `docker-compose.yml` file is available, you can spin up all dependencies:
   ```bash
   docker-compose up
   ```
...
5. Access services through the Gateway:
   ```bash
   curl -X POST http://localhost:8080/orders -d '{"item_id": 123, "quantity": 2}'
   ```
...
s and adapters) design pattern to interact with external payment systems. Stripe is currently implemented as the payment handler.
- **Distributed Design**: Each service is isolated, owns its database, and communicates via gRPC or message queues.
- **Telemetry**: Jaeger provides end-to-end tracing for request flows across the microservices.
